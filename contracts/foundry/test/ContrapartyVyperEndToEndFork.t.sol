...
contract ContrapartyVyperEndToEndForkTest is TestBase {
    ...
    address private uniswapV3AmmAddr;
    address private uniswapV2AmmAddr;
    address private aerodromeAmmAddr;
    ...
    function setUp() public {
        ...
        uniswapV3AmmAddr = vm.deployCode("src/UniswapV3PropAMM.vy", abi.encode(UNISWAP_V3_VIEW_QUOTER));
        IUniswapV3PropAMM(uniswapV3AmmAddr).register_pool(UNISWAP_V3_WETH_USDC_POOL_500, V3_FEE_500);
        IUniswapV3PropAMM(uniswapV3AmmAddr).register_pool(UNISWAP_V3_WETH_USDC_POOL_3000, V3_FEE_3000);
        uniswapV2AmmAddr = vm.deployCode("src/UniswapV2PropAMM.vy", abi.encode(UNISWAP_V2_FACTORY));
        aerodromeAmmAddr = vm.deployCode("src/AerodromePropAMM.vy", abi.encode(AERODROME_FACTORY));

        contraparty = IContrapartyVyper(contrapartyAddr);

        contraparty.register_amm(uniswapV3AmmAddr);
        contraparty.register_amm(uniswapV2AmmAddr);
        contraparty.register_amm(aerodromeAmmAddr);

        _fundUserWeth();
    }

    function testForkVyper_EndToEndDeployQuoteAndSwap() public {
        uint256 amountIn = 0.02 ether;
        uint256 cpQuote = contraparty.quote(WETH, USDC, amountIn);
        assertGt(cpQuote, 0, "contraparty quote is zero");
        uint256 minOut = (cpQuote * 995) / 1000;
        ...
        assertGt(usdcOut, 0, "contraparty swap amount out is zero");
        assertGt(usdcAfter, usdcBefore, "contraparty swap did not increase USDC");
        assertTrue(wethAfter < wethBefore, "contraparty swap did not spend WETH");
    }

    function testForkVyper_EndToEndLargeSwap() public {
        uint256 amountIn = LARGE_SWAP_AMOUNT;
        uint256 cpQuote = contraparty.quote(WETH, USDC, amountIn);
        assertGt(cpQuote, 0, "contraparty large quote is zero");
        uint256 minOut = (cpQuote * 995) / 1000;
        ...
        assertGt(usdcOut, 0, "contraparty large swap amount out is zero");
        assertGt(usdcAfter, usdcBefore, "contraparty large swap did not increase USDC");
        assertTrue(wethAfter < wethBefore, "contraparty large swap did not spend WETH");
    }
